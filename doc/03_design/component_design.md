...
- 入力：`src/articles/hitokuchi/{bamei}/index.md`（馬データ＋本文）と `src/data/hitokuchiHorses.ts`（クラブ定義）
- 出力：一口馬主ポートフォリオ画面
- 処理内容：
  1. タブ（出資馬 / 分析）を切替
...
- 概要：出資馬ごとの日記・観戦記・写真・イベントを表示する
- 入力：URL の `{bamei}` と `src/articles/hitokuchi/{bamei}/index.md` の該当馬データ
- 出力：愛馬日記画面
- 処理内容：
  1. `bamei` で馬を特定。未存在時は `NotFound`
  2. 記事本文（ビルド時に HTML 化済み）を描画
  3. ヘッダーに馬名・クラブ・クラス・戦績・獲得賞金を表示
  4. 写真プレースホルダと出走・イベント一覧を表示
...
| AibaDiary | 出資馬ごとの日記・写真・イベント | 愛馬日記、記事表示相当 |
